...
def check_apostropheError(text):
    nlp = tagger(text)
    error_count = 0
    flag = False  # acts as a flag to check if the final if condition is executed successfully
    correct_text = ""
    suffixList = ["m", "re", "t", "s", "ll", "ve", "d"]
    for sent in nlp:
        for index in range(len(sent)):
            if index < len(sent) - 1 and sent[index][1] in ['WRB', 'MD', 'PRP', 'PRP$', 'VBZ', 'RB', 'NNP', 'VBP', 'NN',
                                                            'VBD', 'NNS', 'VB', 'JJ'] \
                    and sent[index + 1][1] in ['VBP', 'VBG', 'VBD', 'JJ', 'VB', 'NN', 'IN']:
                if sent[index][1] == "VBZ" and sent[index + 1][1] == "JJ" or sent[index][1] == "MD" and sent[index + 1][1] == "VB" \
                        or sent[index][1] == "VB" and sent[index + 1][1] == "JJ":  # special case of is(i's)
                    ...
                    for suffix in suffixList:  # loop traversing through suffixList
                        if sent[index][0].endswith(suffix):  # checking if the token ends with a particular suffix
                            if suffix == 't':
                                if sent[index][0].endswith("nt"):  # special case of nt
                                    length = len(suffix)
